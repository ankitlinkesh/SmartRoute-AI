...
def simulate_routes(
    *,
    ...
    routing_policy_name: str = "balanced",
) -> Dict:
    ...
    disabled_routes = [route for route in routes if int(route.get("bus_number", -1)) in disabled_set]
    impacted_students = sum(int(route.get("total_students", 0)) for route in disabled_routes)

    default_capacity_profile = build_capacity_profile(
        bus_capacity=bus_capacity,
        occupancy_percent=occupancy_percent,
        overflow_enabled=overflow_enabled,
        overflow_limit=overflow_limit,
    )
    planned_capacity = int(default_capacity_profile["planned_capacity"])
    usable_capacity = int(default_capacity_profile["usable_capacity"])

    per_bus_map = per_bus_actual_capacities or {}
    redistributed_students = 0
    remaining_impacted_students = int(impacted_students)
    for route in sorted(active_routes, key=lambda item: int(item.get("bus_number", 0))):
        bus_num = int(route.get("bus_number", 0))
        actual_capacity = max(1, int(per_bus_map.get(bus_num, route.get("actual_capacity", bus_capacity))))
        bus_profile = build_capacity_profile(
            bus_capacity=actual_capacity,
            occupancy_percent=occupancy_percent,
            overflow_enabled=overflow_enabled,
            overflow_limit=overflow_limit,
        )
        bus_usable_capacity = int(bus_profile["usable_capacity"])
        current_load = int(route.get("total_students", 0))
        spare = max(0, bus_usable_capacity - current_load)
        added = min(spare, remaining_impacted_students)
        if added > 0:
            route["total_students"] = current_load + added
            route["simulation_added_students"] = added
            redistributed_students += added
            remaining_impacted_students -= added
        else:
            route["simulation_added_students"] = 0
        route["simulation_usable_capacity"] = bus_usable_capacity
        route["simulation_planned_capacity"] = int(bus_profile["planned_capacity"])

    current_students = sum(int(route.get("total_students", 0)) for route in active_routes)
    simulated_buses_used = len(active_routes)
    network_capacity = sum(int(route.get("simulation_usable_capacity", usable_capacity)) for route in active_routes)
    planned_network_capacity = sum(int(route.get("simulation_planned_capacity", planned_capacity)) for route in active_routes)
    overflow_passengers = max(0, current_students - planned_network_capacity)
    unassigned = max(0, remaining_impacted_students)
    overloaded_buses = sum(
        1
        for route in active_routes
        if int(route.get("total_students", 0)) > int(route.get("simulation_usable_capacity", usable_capacity))
    )
    ...
    warnings: List[str] = []
    if impacted_students > 0:
        warnings.append(
            f"Simulation disabled {len(disabled_routes)} bus(es); {redistributed_students} students reassigned and {unassigned} left unassigned."
        )
    if unassigned > 0:
    ...
    return {
        ...
        "metrics": {
            ...
            "overflow_capacity": int(default_capacity_profile["overflow_capacity"]),
            "usable_capacity": int(usable_capacity),
            "bus_reduction": max(0, len(routes) - int(simulated_buses_used)),
            "students_impacted_by_disabled_buses": int(impacted_students),
            "students_reassigned_from_disabled_buses": int(redistributed_students),
            ...
            "routing_policy": str(routing_policy_name or "balanced"),
        },
        "warnings": warnings,
    }
